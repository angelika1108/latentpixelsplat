...
# From https://github.com/CompVis/latent-diffusion/blob/a506df5756472e2ebaf9078affdde2c4f1502cd4/ldm/modules/diffusionmodules/model.py
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 attn_type="vanilla", embed_dim=3, **ignorekwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
        self.num_resolutions = len(ch_mult)
...
if __name__ == '__main__':
    ...
    print(weights_noattan['state_dict'].keys()) 
    ...
    encoder3 = Encoder(ch=128, out_ch=3, ch_mult=(1,2,4), num_res_blocks=2,
                    ...
                    use_linear_attn=False, attn_type=attn_type, embed_dim=3)
    ...
    for k in decoder3.state_dict():
        if (k != 'post_quant_conv.weight') and (k != 'post_quant_conv.bias'):
    ...
    latent_cat = (latent_cat - latent_cat.min()) / (latent_cat.max() - latent_cat.min())
    print("latents", summarize_tensor(latent_cat[0]))
    latent_cat_im = TF.to_pil_image(latent_cat[0].to(dev))
    latent_cat_im.save('outputs/latent_cat.png')

    # Decode latent image
    decoded_cat = decoder3(latent_cat)
    decoded_cat = (decoded_cat - decoded_cat.min()) / (decoded_cat.max() - decoded_cat.min())
    print("decoded image", summarize_tensor(decoded_cat[0]))
    decoded_cat_im = TF.to_pil_image(decoded_cat[0].to(dev))
