#!/bin/bash
#SBATCH --job-name=acid_tiny_rnd_6l                   # name of job
...
#SBATCH --output=slurm_logs/acid_tiny_rnd_6l_%j.output   # name of output file
#SBATCH --error=slurm_logs/acid_tiny_rnd_6l_%j.error     # name of error file (here, in common with the output file)
...
# Code execution
srun python3 -m src.main +experiment=acid exp_name=${EXP_NAME} hydra.run.dir=${RUN_DIR} decoder_latent_dim=6 model.encoder.d_latent=6 model.decoder.d_latent=6 trainer.devices=4 trainer.num_nodes=1 data_loader.train.batch_size=1 wandb.mode=offline wandb.tags=[acid,256x256,d_latent] loss.lpips.apply_after_step=40000 trainer.max_steps=80000 checkpointing.every_n_train_steps=10000
